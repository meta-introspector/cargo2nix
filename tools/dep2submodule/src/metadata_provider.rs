...
impl CargoMetadataProvider for RealCargoMetadataProvider {
    fn provide_metadata(&self, manifest_path: &Path) -> Result<Metadata> {
        let metadata = cargo_metadata::MetadataCommand::new()
            ...
            .no_deps() // Ensure no external dependencies are fetched
            .exec()?;
...
impl CargoMetadataProvider for DummyCargoMetadataProvider {
    fn provide_metadata(&self, manifest_path: &Path) -> Result<Metadata> {
        ...
        Ok(Metadata {
            packages: vec![
                Package {
                    id: PackageId { repr: "dummy-package-id-2".to_string() },
                    name: "dummy-package-2".to_string(),
            ...
            workspace_root: manifest_path.parent().unwrap_or(Path::new("/")).to_path_buf(),
        })
    }
}
