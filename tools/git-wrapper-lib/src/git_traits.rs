...
use crate::git_types::{RollupLock, SubmoduleInfo, SubmoduleStat};
...
pub trait GitRepositoryOperations {
    ...
    fn git_submodule_update(&self, repo_path: &Path, init: bool, recursive: bool)
        -> Result<Output>;
...
pub trait Execv: Send + Sync {
    fn execv(&self, program: &OsStr, args: &[&OsStr], current_dir: Option<&Path>)
        -> Result<Output>;
}
