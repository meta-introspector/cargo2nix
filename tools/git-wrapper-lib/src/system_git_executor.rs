...
impl SystemGitExecutor {
    pub fn new(git_executable_path: PathBuf, executor: Arc<dyn Execv + Send + Sync>, rollup_lock: Arc<Mutex<RollupLock>>, root_dir: PathBuf) -> Self {
        SystemGitExecutor {
            git_executable_path,
            executor,
            rollup_lock,
...
impl GitExecutor for SystemGitExecutor {
    ...
    fn list_submodules(&self, root_dir: &Path) -> Result<Vec<(String, PathBuf)>> {
        println!("Executing git submodule status --recursive in ?{:?}", root_dir);
        let program = self.git_executable_path.as_os_str();
        let args = &[
            OsStr::new("submodule"),
            OsStr::new("status"),
        ...
        for line in stdout.lines() {
            ...
            if parts.len() >= 2 {
                ...
                let config_output = self.executor.execv(
                    program,
                    config_args,
                    Some(root_dir),
                )?;
                ...
                if config_output.status.success() {
                    let url = String::from_utf8_lossy(&config_output.stdout).trim().to_string();
                    if !url.is_empty() {
                        submodules.push((url, submodule_path));
                    }
                } else {
                    eprintln!("Warning: Could not get URL for submodule {}. Stderr: {}", path_str, String::from_utf8_lossy(&config_output.stderr));
                }
            }
        }
        Ok(submodules)
    ...
    fn get_submodule_head_and_workdir_hash(&self, path: &Path) -> Result<SubmoduleStat> {
        ...
        let head_commit_output = self.executor.execv(
            program,
            head_commit_args,
            None,
        )?;
        ...
        let workdir_status_output = self.executor.execv(
            program,
            workdir_status_args,
            None,
        )?;
