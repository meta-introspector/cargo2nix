...
impl SubmoduleManager {
    ...
    pub fn add_submodule(&self, repo_url: &str, path: &Path) -> Result<()> {
        ...
        self.git_wrapper.git_repo_operations().git_submodule_add(&self.root_dir, repo_url, path.to_str().unwrap(), None, None)?; // Changed
        Ok(())
    }

    pub fn update_submodules(&self) -> Result<()> {
        self.git_wrapper.git_repo_operations().git_submodule_update(&self.root_dir, true, true)?; // Changed
        Ok(())
    }

    pub fn status_submodules(&self) -> Result<String> {
        let output = self.git_wrapper.git_repo_operations().git_submodule_status(&self.root_dir)?; // Changed
