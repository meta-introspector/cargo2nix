...
fn main() -> Result<()> {
    ...
    let workspace_members: Vec<&Package> = metadata.workspace_members.iter()
        .filter_map(|id| metadata.packages.iter().find(|pkg| &pkg.id == id))
        .collect();

    let target_package_ids: Vec<cargo_metadata::PackageId> = if let Some(pkg_name) = &args.package_name {
    ...
    let mut inverse_graph: HashMap<cargo_metadata::PackageId, Vec<cargo_metadata::PackageId>> = HashMap::new();
    for package in &metadata.packages {
        for dep in &package.dependencies {
            // Find the actual PackageId for the dependency name
            if let Some(dep_pkg) = metadata.packages.iter().find(|p| p.name.as_str() == dep.name.as_str()) {
                inverse_graph.entry(dep_pkg.id.clone()).or_default().push(package.id.clone());
            }
        }
    }

    // 3. Build a forward dependency graph (package -> [direct_dependencies])
    let mut forward_graph: HashMap<cargo_metadata::PackageId, Vec<cargo_metadata::PackageId>> = HashMap::new();
    for package in &metadata.packages {
        for dep in &package.dependencies {
            if let Some(dep_pkg) = metadata.packages.iter().find(|p| p.name.as_str() == dep.name.as_str()) {
                forward_graph.entry(package.id.clone()).or_default().push(dep_pkg.id.clone());
            }
        }
    }
        ...
        .filter(|e| e.file_type().is_file() && e.file_name() == "Cargo.toml")
    {
        ...
        let submodule_package_metadata = MetadataCommand::new()
            ...
            .no_deps()
                    .exec()
                    .context(format!("Failed to get metadata for submodule Cargo.toml: {:?}", cargo_toml_path))?
                    ;
                    ...
                    if let Some(pkg) = submodule_package_metadata.packages.first() {
                        let relative_path = diff_paths(cargo_toml_path.parent().unwrap(), &project_root)
                            .context(format!("Failed to get relative path for submodule: {:?}", cargo_toml_path))?;
                ...
                for current_root_pkg_id in &inverted_dependencies {
                    let current_root_pkg = metadata.packages.iter().find(|p| p.id == *current_root_pkg_id).unwrap();
                    ...
                    std::fs::create_dir_all(&current_workspace_output_dir).context(format!("Failed to create output directory for {}", current_root_pkg_name))?;
                    ...
                    for member_name in &members_for_this_workspace {
                        if let Some(sub_path) = submodule_paths.get(member_name.as_str()) {
                            let relative_to_output = diff_paths(project_root.join(sub_path), &current_workspace_output_dir)
                                .context(format!("Failed to get relative path for member {}", member_name))?;
                    ...
                    for (dep_name, sub_path) in &direct_submodule_deps {
                        let relative_to_output = diff_paths(project_root.join(sub_path), &current_workspace_output_dir)
                            .context(format!("Failed to get relative path for workspace dependency {}", dep_name))?;
                        let mut dep_table = Table::new();
                        dep_table.insert("path", value(relative_to_output.to_string_lossy().to_string()));
                        ...
                        if let Some(sub_path) = submodule_paths.get(pkg.name.as_str()) {
                            if pkg.name.as_str() != current_root_pkg_name.as_str() &&
                               ...
                               !direct_submodule_deps.contains_key(pkg.name.as_str()) {
                                transitive_submodule_deps_for_patching.insert(pkg.name.to_string(), sub_path.clone());
                            }
                        } else {
                            // It's a non-local transitive dependency, check if it needs to be added as a submodule
                            if let Some(source) = &pkg.source {
                                if source.repr.starts_with("git+") {
                                    let mut full_repo_url = source.repr.trim_start_matches("git+").to_string();
                    ...
                    for (pkg_name, original_path) in &transitive_submodule_deps_for_patching {
                        let relative_to_output = diff_paths(project_root.join(original_path), &current_workspace_output_dir)
                            .context(format!("Failed to get relative path for patch entry {}", pkg_name))?;
                        let mut dep_table = Table::new();
                        dep_table.insert("path", value(relative_to_output.to_string_lossy().to_string()));
                    ...
                    std::fs::write(&new_cargo_toml_path, doc.to_string().as_bytes())
                        .context(format!("Failed to write new Cargo.toml for {}", current_root_pkg_name))?;
            
                    println!("Generated new workspace for '{}' at: {:?}", current_root_pkg_name, current_workspace_output_dir);
