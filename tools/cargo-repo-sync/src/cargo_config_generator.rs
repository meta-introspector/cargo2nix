...
use cargo_metadata::MetadataCommand; // Add cargo_metadata import
...
pub fn parse_members_file(project_root: &Path) -> Result<WorkspaceInfo> {
    ...
    for submodule in repo.submodules().context("Failed to read submodules")? {
        ...
        if submodule_cargo_toml.exists() {
            let metadata = MetadataCommand::new()
                .manifest_path(&submodule_cargo_toml)
                .no_deps()
                .exec()
                .with_context(|| format!("Failed to get cargo metadata for submodule manifest: {:?}", submodule_cargo_toml))?;
            ...
            .filter(|e| e.file_type().is_file() && e.file_name() == "Cargo.toml")
        {
            ...
            let metadata = MetadataCommand::new()
                .manifest_path(&sub_cargo_toml_path)
                .no_deps()
                .exec()
                .with_context(|| format!("Failed to get cargo metadata for sub-submodule manifest: {:?}", sub_cargo_toml_path))?;
        ...
        if !member_crates.is_empty() {
            workspace_info.push(SubmoduleWorkspaceInfo {
                submodule_base_path_rel: submodule_path_rel.to_path_buf(),
                member_crates,
            });
        }
    }
...
pub fn update_config_toml(config_toml_path: &Path, new_patches: &GeneratedPatches) -> Result<()> {
