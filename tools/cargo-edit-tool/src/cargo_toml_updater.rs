...
impl RealCargoTomlUpdater {
    pub fn new(
        ...
        RealCargoTomlUpdater { cargo_metadata_provider, cargo_edit_executor }
    }
}
...
impl CargoTomlUpdater for RealCargoTomlUpdater {
    fn generate_deps_from_submodules(&self, project_root: &Path) -> Result<Vec<String>> {
            ...
            .filter(|e| e.file_type().is_file() && e.file_name() == "Cargo.toml")
        {
            ...
            let metadata = self.cargo_metadata_provider.provide_metadata(cargo_toml_path)?;
    ...
    fn update_cargo_toml(&self, cargo_toml_path: &Path, generated_deps_content: &[String]) -> Result<()> {
        ...
        self.cargo_edit_executor.apply_patches(cargo_toml_path, patches)?;
