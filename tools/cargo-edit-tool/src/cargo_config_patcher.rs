...
#[cfg(feature = "nix_generation")] // Conditionally compile RealCargoConfigPatcher
    ...
    fn generate_patches(
        ...
        for line in tree_content.lines() {
            if let Some(captures) = submodule_path_re.captures(line) {
                ...
                let relative_path = diff_paths(&full_path, project_root)
...
impl CargoConfigPatcher for RealCargoConfigPatcher {
    fn generate_patches(
        &self,
        _tree_file: &Path,
        _cargo_lock_file: &Path,
        _config_file: &Path,
        _project_root: &Path,
    ) -> Result<Vec<String>> {
        anyhow::bail!("`CargoConfigPatcher` requires the `nix_generation` feature to be enabled.");
    }
}