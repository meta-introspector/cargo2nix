...
#[cfg(feature = "walkdir")]
use walkdir::WalkDir; // Import Regex
...
impl WorkspaceRemover for RealWorkspaceRemover {
    fn remove_workspace_sections(&self, submodules_dir: &Path) -> Result<()> {
        println!(
            "Scanning for Cargo.toml files in submodules under: {}",
            submodules_dir.display()
        );
            ...
            .filter(|e| e.file_type().is_file() && e.file_name() == "Cargo.toml")
        {
            ...
            let content = std::fs::read_to_string(&cargo_toml_path).with_context(|| {
            ...
            if workspace_header_re.is_match(&new_content) {
                new_content = workspace_header_re
                    .replace_all(&new_content, "")
                    .to_string();
                changed = true;
                println!(
                    "  Removed [workspace] header remnants from {}",
                    cargo_toml_path.display()
                );
            }
            ...
            if changed {
                ...
                println!(
                    "  No [workspace] section found or changes needed in {}",
                    cargo_toml_path.display()
                );
            }
        }
