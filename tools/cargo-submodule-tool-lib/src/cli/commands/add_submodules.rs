...
pub fn run_add_submodules_command(args: &AddSubmodulesArgs, cli: &Cli) -> Result<()> {
    ...
    let git_executor: Arc<dyn GitExecutor + Send + Sync> = {
        #[cfg(feature = "git_enabled")]
        {
            Arc::new(PureRustGitExecutor::new(rollup_lock_arc.clone(), args.root_dir.clone())) // Changed root_dir to args.root_dir
        }
        #[cfg(not(feature = "git_enabled"))]
        {
    ...
    let real_file_system_stat = RealFileSystemStat::new(git_executor.clone(), rollup_lock_arc.clone(), args.root_dir.clone()); // Changed root_dir to args.root_dir

    let _file_system_writer: Box<dyn FileSystemWriter> = if cli.dry_run {
        Box::new(CachedFileSystemWriter::new(Arc::new(real_file_system_stat.clone()), rollup_lock_arc.clone(), args.root_dir.clone())) // Changed root_dir to args.root_dir
    } else {
