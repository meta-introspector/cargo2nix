...
struct SafeSolanaMapper {
    accounts: Vec<SolanaCargoAccount>,
}

impl SafeSolanaMapper {
    fn new() -> Self {
        Self {
            accounts: Vec::new(),
        }
    ...
    fn generate_solana_address(&self, content_hash: &str, crate_name: &str) -> String {
        ...
        let seed = format!("cargo:{}:{}", content_hash, crate_name);
        let mut hasher = DefaultHasher::new();
        seed.hash(&mut hasher);
        
        // Generate 44-character base58-like address (Solana format)
    ...
    fn generate_account_report(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, account) in self.accounts.iter().enumerate() {
            if i < 15 { // Show first 15
                ...
                let account_short = SolanaCargoAccount::safe_truncate(&account.account_address, 32);
                let content_short = SolanaCargoAccount::safe_truncate(&account.content_hash, 16);
                let git_obj_short = SolanaCargoAccount::safe_truncate(&account.git_object, 16);
                ...
                report.push_str(&format!("- **Account Mapping**: `{}` -> Solana Account `{}`\n", 
                    account.cargo_crate_name, &account_short[..16]));
                
                report.push_str("\n");
            }
        }
        
        if self.accounts.len() > 15 {
        ...
        report.push_str("## Account Statistics\n");
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut mapper = SafeSolanaMapper::new();
    mapper.run()
}
