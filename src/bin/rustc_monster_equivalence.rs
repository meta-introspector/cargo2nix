use cargo2nix::{RustcBlockAnalyzer, MiniZincSolver};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for block in &rustc_blocks {
        println!("{}: prime={}, supersingular_index={}, constraint={}",
    ...
    for (name, monster_element, constraint) in analyzer.transform_all_blocks() {
        println!("{} → Monster element {} (mod 196883), constraint {}",
    ...
    match solver.solve_with_data("./models/rustc_monster_equivalence.mzn", &minizinc_input) {
        Ok(solution) => {
            ...
            println!("Supersingular prime satisfaction: {}", solution.objective >= 108.0);
