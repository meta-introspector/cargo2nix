...
fn analyze_if_as_monster_generator() {
    ...
    println!("   Monster element = {}", (if_phi.wrapping_mul(if_usage)) % 196883);
...
fn find_monster_structure() {
    ...
    for symbol in &symbols {
        power_groups.entry(symbol.power_of_2).or_default().push(symbol);
    }
