use cargo2nix::{Monster108Constraints, MiniZincSolver};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("First 10 primes: {:?}", &monster_system.supersingular_primes[0..10]);
    println!("Last 10 primes: {:?}", &monster_system.supersingular_primes[98..108]);
    ...
    for (i, constraint) in monster_system.constraints.iter().take(10).enumerate() {
        let a_val = constraint.a_coeff.iter().zip(sample_witness.iter()).map(|(a, w)| a * w).sum::<i64>();
        let b_val = constraint.b_coeff.iter().zip(sample_witness.iter()).map(|(b, w)| b * w).sum::<i64>();
        let c_val = constraint.c_coeff.iter().zip(sample_witness.iter()).map(|(c, w)| c * w).sum::<i64>();
        
        let verified = (a_val * b_val) % constraint.supersingular_prime as i64 == c_val % constraint.supersingular_prime as i64;
    ...
    println!("\nSample verification: {}/10 constraints passed", verified_count);
    ...
    println!("Supersingular primes: {}", monster_system.supersingular_primes.len());
