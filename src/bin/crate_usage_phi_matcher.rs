...
impl PhiUsageMatcher {
    ...
    fn analyze_crate_usage(&mut self, crate_name: &str, file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn calculate_usage_similarity(&mut self) {
        ...
        for i in 0..crate_names.len() {
            for j in i + 1..crate_names.len() {
                ...
                self.similarity_matrix.insert((crate_a.clone(), crate_b.clone()), similarity);
                
                println!("  {} ↔ {}: {:.2}% similar", crate_a, crate_b, similarity * 100.0);
            }
        }
    }
    ...
    fn compute_phi_similarity(&self, crate_a: &str, crate_b: &str) -> f64 {
        ...
        let sig_diff = (profile_a.usage_phi_signature as i64 - profile_b.usage_phi_signature as i64).abs() as f64;
        let sig_similarity = 1.0 / (1.0 + sig_diff / 10000.0);
        
        // Pattern overlap similarity
        let common_phis = profile_a.usage_pattern.iter()
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if matches.is_empty() {
        ...
        for (crate_a, crate_b, similarity) in &matches {
            println!("🎯 {} ≈ {} ({:.1}% similar)", crate_a, crate_b, similarity * 100.0);
