...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let submodule_output = Command::new("git").args(&["submodule", "status"]).output()?;
    let submodule_data = String::from_utf8_lossy(&submodule_output.stdout);
    
    println!("1. [9] solana-rustc | real | {} | agave | {}/compiler | git:{}", 
    ...
    for line in submodule_data.lines().take(5) {
        ...
        if parts.len() >= 2 {
            ...
            if matches!(name, "rustc-build-sysroot" | "rustc-demangle" | "cargo" | "serde" | "allocator-api2") {
                let branch = get_git_branch(path).unwrap_or_else(|_| "unknown".to_string());
                println!("{}. [8] {} | real | {} | real | {} | git:{}", 
                    order, name, branch, path, &hash[..8]);
                order += 1;
            }
        }
    }
