...
fn main() {
    ...
    match integration.solve_monster_lattice(lattice_size) {
        Ok(solution) => {
            ...
            println!("  Monster Group Constraints: {}", if is_valid { "✅ Valid" } else { "❌ Invalid" });
            
            let sum: i32 = solution.iter().sum();
            println!("  Sum: {} (mod 24 = {})", sum, sum % 24);
            
            let unique_count = solution.iter().collect::<std::collections::HashSet<_>>().len();
    ...
    match integration.solve_resource_allocation(&resources, &constraints) {
        Ok(allocation) => {
            ...
            println!("  Total Allocated: {} / {} available", total_allocated, total_available);
            
            let allocation_sum = allocation.iter().sum::<i32>();
            println!("  Modular Check: {} mod 24 = {}", allocation_sum, allocation_sum % 24);
        },
        Err(e) => {
...
fn print_model_summary(model: &str) {
    ...
    let constraint_count = lines.iter().filter(|line| line.contains("constraint")).count();
    ...
    println!("    Lines: {}, Constraints: {}, Variables: {}, Includes: {}", 
             lines.len(), constraint_count, variable_count, include_count);
...
fn verify_vendorization_status() {
    ...
    for path in &vendor_paths {
        ...
        println!("    {}: {}", path, if exists { "✅ Found" } else { "❌ Missing" });
    }
