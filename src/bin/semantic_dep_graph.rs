...
impl RocksDBSemanticGraph {
    ...
    fn ingest_from_rocksdb(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.add_semantic_node("rustc_middle", "rustc_middle", "1.75.0", vec!["rustc_core".to_string()], 2, "compiler:ir");
        self.add_semantic_node("rustc_driver", "rustc_driver", "1.75.0", vec!["rustc_middle".to_string()], 3, "compiler:driver");
        self.add_semantic_node("cargo", "cargo", "0.75.0", vec!["rustc_driver".to_string()], 4, "build:manager");
    ...
    fn add_semantic_node(&mut self, id: &str, name: &str, version: &str, deps: Vec<String>, layer: usize, sem_type: &str) {
        self.nodes.insert(id.to_string(), SemanticNode {
            id: id.to_string(),
    ...
    fn format_semantic_types_query(&self) -> String {
        ...
        for node in self.nodes.values() {
            types.entry(node.semantic_type.clone())
    ...
    fn format_topological_query(&self) -> String {
        ...
        for (i, node) in sorted.iter().enumerate() {
            result.push_str(&format!("    {}. [layer:{}] {} | {} | {}\n", 
                i + 1, node.layer, node.crate_name, node.version, node.semantic_type));
        }
