...
impl RustcUsageResolver {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if let Some(call_pos) = trimmed.find('(') {
                ...
                if let Some(last_space) = before_paren.rfind(' ') {
                    ...
                    if call_name.chars().all(|c| c.is_alphanumeric() || c == '_') && !call_name.is_empty() {
            ...
            if trimmed.starts_with("use ") {
                let use_part = trimmed.strip_prefix("use ").unwrap_or("").strip_suffix(';').unwrap_or("");
    ...
    fn add_cargo_crate(&mut self, crate_name: &str, exports: Vec<&str>) {
        ...
        self.cargo_imports.insert(crate_name.to_string(), cargo_import);
        println!("Added cargo crate: {} with {} exports", crate_name, exports.len());
    }
    ...
    fn resolve_all_usages(&mut self) {
        ...
        for usage in &self.unresolved {
            ...
            if let Some((crate_name, export_name)) = best_match {
                if best_score > 50 { // Threshold for good match
                    let phi = self.cargo_imports[&crate_name].phi_values[&export_name];
                    self.resolved.insert(usage.name.clone(), (crate_name.clone(), phi));
                    println!("  {} → {}::{} (φ = {}, score: {})", 
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    resolver.add_cargo_crate("serde", vec!["serialize", "deserialize", "Serialize", "Deserialize"]);
    resolver.add_cargo_crate("tokio", vec!["spawn", "block_on", "Runtime", "sleep"]);
    resolver.add_cargo_crate("std", vec!["println", "format", "write", "read_to_string", "HashMap"]);
    resolver.add_cargo_crate("minizinc", vec!["solve", "constraint", "variable", "output"]);
