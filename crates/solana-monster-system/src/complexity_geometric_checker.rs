...
impl ComplexityGeometricChecker {
    ...
    pub fn check_complexity_bounds(&self, constraint: &R1CSConstraint, witness: &[i64]) -> bool {
        ...
        let circuit_depth =
            constraint.a_coeff.len() + constraint.b_coeff.len() + constraint.c_coeff.len();
    ...
    pub fn verify_geometric_equivalence(&self, phi_equiv: &GeometricEquivalence) -> bool {
        ...
        (left_sum % phi_equiv.equivalence_prime as i64)
            == (right_sum % phi_equiv.equivalence_prime as i64)
    }

    pub fn generate_constraint_from_bounds(&self, constraint_id: usize) -> R1CSConstraint {
        let prime_factor =
            self.monster_prime_factors[constraint_id % self.monster_prime_factors.len()];

        R1CSConstraint {
            id: constraint_id,
            reason: format!(
                "Complexity Bound (w={}, ℓ={}) + Geometric Equivalence",
