...
fn process_rust_file(path: &Path, global_symbols: &mut HashMap<String, SymbolUsage>) {
    if let Ok(content) = fs::read_to_string(path) {
        ...
        let module_name = format!("{}::{}", crate_name, 
...
fn count_symbols_in_line(line: &str, module_name: &str, crate_name: &str, global_symbols: &mut HashMap<String, SymbolUsage>) {
    // Common Rust symbols to track
    let rust_symbols = [
        "Vec", "HashMap", "String", "Option", "Result", "Box", "Rc", "Arc",
        "Serialize", "Deserialize", "Clone", "Debug", "Display", "Error",
        "spawn", "Runtime", "Future", "Stream", "async", "await",
        "println", "format", "panic", "unwrap", "expect", "match",
    ];
...
fn update_symbol_usage(symbol: &str, module_name: &str, crate_name: &str, count: u64, 
                      global_symbols: &mut HashMap<String, SymbolUsage>) {
    let entry = global_symbols.entry(symbol.to_string()).or_insert_with(|| SymbolUsage {
    ...
    *entry.crate_distribution.entry(crate_name.to_string()).or_insert(0) += count;
}
...
fn analyze_by_crate(global_symbols: &HashMap<String, SymbolUsage>) -> Vec<CrateAnalysis> {
    ...
    for symbol_usage in global_symbols.values() {
        for (crate_name, &usage_count) in &symbol_usage.crate_distribution {
            let entry = crate_analysis.entry(crate_name.clone()).or_insert_with(|| CrateAnalysis {
            ...
            entry.top_symbols.push((symbol_usage.symbol.clone(), usage_count));
        }
    }
...
fn main() {
    ...
    for (i, symbol) in sorted_symbols.iter().take(20).enumerate() {
        println!("{}. {} (φ: {}, usage: {}, modules: {}, crates: {}, complexity: {:.0})",
                i + 1, symbol.symbol, symbol.phi_value, symbol.usage_count,
                symbol.used_in_modules.len(), symbol.crate_distribution.len(),
    ...
    for (i, analysis) in crate_analyses.iter().take(15).enumerate() {
        println!("{}. {} (complexity: {:.0}, symbols: {})",
                i + 1, analysis.crate_name, analysis.total_complexity, analysis.symbol_count);
        
        println!("   Top symbols: {:?}", 
                analysis.top_symbols.iter().map(|(s, c)| format!("{}({})", s, c)).collect::<Vec<_>>());
    }
    
    println!("\n=== Cross-Crate Symbol Distribution ===");
    for symbol in sorted_symbols.iter().take(10) {
        if symbol.crate_distribution.len() > 3 {
            println!("🔧 {} used across {} crates:", symbol.symbol, symbol.crate_distribution.len());
