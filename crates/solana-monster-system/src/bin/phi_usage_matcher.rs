...
fn calculate_usage_phi(usage_pattern: &[String]) -> u64 {
    ...
    for usage in usage_pattern {
        let usage_hash = usage.bytes().fold(5381u64, |acc, b| {
                ...
                if let Ok(content) = fs::read_to_string(&path) {
                    ...
                    for line in content.lines() {
                        if let Some(dep) = extract_dependency(line) {
                            let entry =
                                usage_map.entry(dep.clone()).or_insert_with(|| CrateUsage {
...
fn find_usage_matches(usages: &[CrateUsage]) -> Vec<UsageMatch> {
    ...
    for i in 0..usages.len() {
        for j in (i + 1)..usages.len() {
            ...
            if usage1.crate_name == usage2.crate_name {
                continue;
            }
            ...
            let match_confidence =
                (phi_similarity * 0.4) + (usage_overlap * 0.4) + (freq_ratio * 0.2);
...
fn main() {
    ...
    for (i, m) in matches.iter().take(20).enumerate() {
        println!(
            "{}. {} ↔ {} (confidence: {:.2}, φ-sim: {:.2}, overlap: {:.2})",
            i + 1,
            m.crate1,
            m.crate2,
            m.match_confidence,
    ...
    for (phi_sig, group) in phi_groups {
        if group.len() > 1 {
            println!(
                "🧮 φ = {}: {} crates with identical usage patterns",
                phi_sig,
                group.len()
            );
            for usage in group {
