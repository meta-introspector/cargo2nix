...
impl CargoCratesDB2 {
    ...
    fn process_cargo_toml(&mut self, cargo_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!("CRATE: {} → CAS: {} ({} deps)", name, cas_address, dependencies.len());
    ...
    fn calculate_crate_cas(&self, name: &str, version: &str, deps: &[String], features: &[String]) -> u64 {
    ...
    fn show_database(&self) {
        ...
        for (cas, crate_info) in &self.crates {
            println!("📦 {} v{} (CAS: {})", crate_info.name, crate_info.version, cas);
