...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for binding in bindings {
        println!("{} | exports:{} imports:{} | modular_index:{}", 
            binding.crate_name,
            binding.exports.len(),
            binding.imports.len(),
...
fn get_real_trait_bindings() -> Vec<TraitBinding> {
    ...
    let trait_data = vec![
        ("rust", vec!["Clone", "Copy", "Debug", "Default", "Drop"], vec!["std::fmt", "std::mem"]),
        ("rustc-demangle", vec!["Demangle"], vec!["std::fmt"]),
        ("cargo", vec!["Serialize", "Deserialize"], vec!["serde", "std::collections"]),
        ("serde", vec!["Serialize", "Deserialize", "Serializer", "Deserializer"], vec!["std::fmt"]),
        ("rust-analyzer", vec!["Analysis", "Completion", "Diagnostic"], vec!["std::collections", "serde", "tokio"]),
    ];
