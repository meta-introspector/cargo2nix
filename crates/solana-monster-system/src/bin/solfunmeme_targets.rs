...
impl SolfunmemeVector {
    ...
    fn to_array(&self) -> [f64; 11] {
        [
            self.eb, self.pr, self.my, self.cb, self.glw, self.swl, self.intp, self.abs, self.geo,
            self.sur, self.fan,
        ]
    }
...
fn main() {
    ...
    println!(
        "  Convergence: {:.6}%",
        (monster_meme.sum() / 1.96883) * 100.0
    );
    ...
    println!(
        "  Target 23/24: {:.4}",
        MathematicalTargets::fraction_23_24()
    );
    println!(
        "  Convergence: {:.2}%",
        (frac_meme.sum() / MathematicalTargets::fraction_23_24()) * 100.0
    );
