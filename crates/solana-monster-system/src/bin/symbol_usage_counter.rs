...
fn calculate_symbol_complexity(
    ...
    for symbol_usage in symbol_stats.values_mut() {
        ...
        symbol_usage.complexity_score =
            (phi_val as f64) * usage_frequency.ln() * module_spread.sqrt();
    }

    symbol_stats
}

fn calculate_module_complexity(
    modules: &mut HashMap<String, ModuleImports>,
    symbol_stats: &HashMap<String, SymbolUsage>,
) {
...
fn main() {
    ...
    for module in sorted_modules.iter().take(10) {
        println!(
            "📁 {} (complexity: {:.1}, symbols: {})",
            module.module_name,
            module.total_complexity,
            module.symbol_usage.len()
        );
