...
impl MonsterPolynomial {
    ...
    fn compute_coefficient(&self, types: &[&str]) -> u64 {
        types.iter().map(|t| t.len() as u64).product::<u64>() % 196883
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  monster_convergence: {:.6}",
        rustc_polynomial.monster_convergence_ratio()
    );
    println!(
        "  is_monster_equivalent: {}",
        rustc_polynomial.is_monster_equivalent()
    );
    println!("  terms: [");
