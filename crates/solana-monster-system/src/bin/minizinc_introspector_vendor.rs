...
fn print_vendorization_status(status: &VendorizationStatus) {
    println!("  Vendor Directory: {}", if status.vendor_directory_exists { "✅ Found" } else { "❌ Missing" });
    println!("  .gitmodules Config: {}", if status.gitmodules_configured { "✅ Configured" } else { "❌ Not configured" });
    println!("  Correct Repository: {}", if status.correct_repository { "✅ Correct" } else { "❌ Incorrect" });
    println!("  asciicast_processor: {}", if status.asciicast_processor_exists { "✅ Found" } else { "❌ Missing" });
    println!("  Submodule Initialized: {}", if status.submodule_initialized { "✅ Initialized" } else { "❌ Not initialized" });
}
...
fn test_solver_integration(integration: &MiniZincIntrospectorIntegration, model: &str) {
    match integration.run_introspector_solver(model) {
        Ok(result) => {
            ...
            println!("  🧮 Monster Group Valid: {}", if result.monster_group_valid { "✅ Yes" } else { "❌ No" });
        },
        Err(e) => {
...
fn test_monster_group_verification() {
    ...
    // Test uniqueness
    let unique_count = test_elements.iter().collect::<std::collections::HashSet<_>>().len();
    let uniqueness_check = unique_count == test_elements.len();
    println!("  Uniqueness: {} unique / {} total ({})", 
             unique_count, 
             test_elements.len(),
             if uniqueness_check { "✅ All unique" } else { "❌ Duplicates" });
    
    // Test bounds
    let bounds_check = test_elements.iter().all(|&x| x >= 0 && x < MONSTER_GROUP_ORDER as i32);
    println!("  Bounds Check: {}", if bounds_check { "✅ Within Monster Group" } else { "❌ Out of bounds" });
}
