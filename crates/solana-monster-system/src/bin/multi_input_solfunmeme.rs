...
impl MultiInputSolfunmeme {
    ...
    fn ingest_rust_code(&mut self, crate_name: &str, code: &str) {
        ...
        let monster_factor = (2_u64.pow((code.matches("struct").count() as u32).min(10))
            ...
            + 71 * code.matches("fn").count() as u64)
            % 196883;
...
fn main() {
    ...
    // Ingest Rust code
    system.ingest_rust_code(
        "serde",
        "struct Serialize { fn serialize() } enum Value { String, Number }",
    );
    system.ingest_rust_code(
        "solana",
        "fn process_instruction() -> Result<()> { Ok(()) }",
    );
