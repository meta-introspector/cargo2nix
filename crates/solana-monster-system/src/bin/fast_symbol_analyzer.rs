...
fn calculate_complexities(symbols: &mut HashMap<String, SymbolStats>) {
    for stats in symbols.values_mut() {
        ...
        stats.complexity =
            (phi_val as f64) * (stats.total_usage as f64).ln() * (stats.crate_count as f64).sqrt();
    }
}
