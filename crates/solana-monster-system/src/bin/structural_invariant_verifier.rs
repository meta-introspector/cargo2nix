use solana_monster_system::{StructuralInvariantChecker, Monster108Constraints, MiniZincSolver};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (consistent, total) = checker.verify_structural_consistency(&monster_system.constraints, &witness);
    ...
    let minizinc_input = MinizincInput {
        ...
        monster_stabilizer: if consistent == total { 108 } else { consistent as i32 },
    };
