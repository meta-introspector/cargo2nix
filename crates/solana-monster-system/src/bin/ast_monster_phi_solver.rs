...
static_count = {};
"#, 
            self.struct_count, self.enum_count, self.fn_count,
            self.impl_count, self.trait_count, self.macro_count,
            self.mod_count, self.use_count, self.const_count, self.static_count
        )
    }
}
...
fn manual_phi_calculation(ast: &RustASTCounts) {
    ...
    println!("  Totient ratio: {:.2}%", (phi_output as f64 / monster_element as f64) * 100.0);
