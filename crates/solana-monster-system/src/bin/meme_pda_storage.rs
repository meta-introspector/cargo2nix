...
impl RocksDBMemeStorage {
    ...
    pub fn create_meme_pda(&mut self, crate_name: &str, version: &str, rustc_sig: &str) -> Result<MemePDA, Box<dyn std::error::Error>> {
        ...
        self.db.put(key.as_bytes(), serde_json::to_vec(&meme_pda)?)?;
    ...
    fn attach_meme_to_crate_in_db(&mut self, crate_name: &str, meme_pda_key: &str) -> Result<(), Box<dyn std::error::Error>> {
        let attachment_key = format!("attachments:{}", crate_name);
        let mut attached_keys: Vec<String> = if let Some(data) = self.db.get(attachment_key.as_bytes())? {
        ...
        self.db.put(attachment_key.as_bytes(), serde_json::to_vec(&attached_keys)?)?;
        Ok(())
    }
    
    pub fn get_crate_memes(&self, crate_name: &str) -> Result<Vec<MemePDA>, Box<dyn std::error::Error>> {
        let attachment_key = format!("attachments:{}", crate_name);
    ...
    fn format_all_memes(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        for item in self.db.iterator(rocksdb::IteratorMode::Start) {
            if count >= 5 { break; } // Limit to 5 for sample output
            
            let (key, value) = item?;
            let key_str = String::from_utf8(key.to_vec())?;
            
            if key_str.starts_with("attachments:") { continue; } // Skip attachment keys
    ...
    fn format_monster_convergence(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        for item in self.db.iterator(rocksdb::IteratorMode::Start) {
            ...
            if key_str.starts_with("attachments:") { continue; }
            
            let pda: MemePDA = serde_json::from_slice(&value)?;
            total_monster += pda.meme_data.monster_factor;
        }
        
        Ok(format!("Monster convergence: {}/196883 ({:.2}%)", 
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create memes
    let mut meme1 = storage.create_meme_pda("serde", "1.0.228", "struct Serialize { fn serialize() }")?;
    let mut meme2 = storage.create_meme_pda("tokio", "1.0.0", "async fn main() { runtime.spawn() }")?;
    let mut meme3 = storage.create_meme_pda("rustc", "1.75.0", "fn compile(ast: AST) -> Binary")?;
    let mut meme4 = storage.create_meme_pda("solana", "1.18.0", "struct Account { lamports: u64 }")?;
