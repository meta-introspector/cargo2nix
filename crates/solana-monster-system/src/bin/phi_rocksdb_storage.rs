        ...
        loop {
            ...
            if let Some(existing_data) = self.db.get(phi_key_bytes.as_bytes())? {
                ...
                *collision_id_entry += 1;
                ...
                println!(
                    "COLLISION at φ = {}: {} vs existing entries (current attempt key: {})",
                ...
                current_phi_key =
                    (original_phi_key + file_hash + decl.collision_id as u64) % 196883;

                println!(
                    "  → Attempting new key φ = {} (collision_id: {})",
                    current_phi_key, decl.collision_id
                );
                ...
                if self
                    .db
                    .get(current_phi_key.to_string().as_bytes())?
                    .is_none()
                {
                    // New key is unique, add it as a new entry
                    self.db.put(
                        current_phi_key.to_string().as_bytes(),
                        serde_json::to_vec(&vec![decl])?,
                    )?;
                ...
                self.db
                    .put(phi_key_bytes.as_bytes(), serde_json::to_vec(&vec![decl])?)?;
                break; // Successfully stored
            }
        }
            ...
            if *count > 0 {
                ...
                if let Some(existing_data) = self.db.get(phi_key_bytes.as_bytes())? {
                    ...
                    for decl in decls {
                        println!(
                            "  - {} ({}) collision_id: {}",
                            decl.name, decl.decl_type, decl.collision_id
                        );
                    }
                }
            }
