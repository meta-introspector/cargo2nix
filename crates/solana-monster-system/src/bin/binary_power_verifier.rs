...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Binary constraints: {}",
        binary_system.binary_constraints.len()
    );
    println!(
        "Bit decomposition length: {}",
        binary_system.bit_decomposition.len()
    );
    ...
    for &value in &test_values {
        ...
        let reconstructed: u64 = encoding
            .iter()
            .enumerate()
    ...
    println!(
        "Constraint system: {} binary R1CS constraints",
        binary_system.binary_constraints.len()
    );
