...
use std::fs;
use std::path::Path;
use syn::{File, Item, ItemStruct, ItemEnum, ImplItem, Type};
...
fn extract_struct_details(item_struct: &ItemStruct, file: &File) -> (Vec<String>, Vec<String>) {
    ...
    for item in &file.items {
        if let Item::Impl(item_impl) = item {
            if let Some(self_ty) = &item_impl.self_ty { // self_ty is &Box<Type>
                let self_ty_deref: &Type = &**self_ty; // self_ty_deref is &Type
                if let Type::Path(type_path) = self_ty_deref {
                    if type_path.path.segments.last().map_or(false, |segment| segment.ident == item_struct.ident) {
    ...
    for item in &file.items {
        if let Item::Impl(item_impl) = item {
            if let Some(self_ty) = &item_impl.self_ty { // self_ty is &Box<Type>
                let self_ty_deref: &Type = &**self_ty; // self_ty_deref is &Type
                if let Type::Path(type_path) = self_ty_deref {
                    if type_path.path.segments.last().map_or(false, |segment| segment.ident == item_enum.ident) {
                        for impl_item in &item_impl.items {
                            if let ImplItem::Fn(impl_item_fn) = impl_item {
                                methods.push(impl_item_fn.sig.ident.to_string());
                            }
...
fn main() {
    ...
    let syn_file = match adapter.parse_file(target_file_path) { // Parse file
        Ok(file) => file,
        Err(e) => {
    ...
    for i in 0..all_types.len() {
        for j in (i + 1)..all_types.len() {
            ...
            if diff < 1000 {
                similar_pairs.push((all_types[i].name().to_string(), all_types[j].name().to_string()));
            }
        }
    }
