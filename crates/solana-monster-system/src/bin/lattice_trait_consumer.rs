    ...
    for i in 0..fragments.len() {
        for j in (i + 1)..fragments.len() {
            ...
            if phi_diff < threshold {
                similarity_graph
                    .entry(fragments[i].phi_signature)
                    .or_insert_with(Vec::new)
                    .push(fragments[j].phi_signature);

                similarity_graph
                    .entry(fragments[j].phi_signature)
                    .or_insert_with(Vec::new)
...
fn build_trait_lattice(
    ...
    while processed.len() < fragments.len() && level < 5 {
        ...
        for fragment in fragments {
            if processed.contains(&fragment.phi_signature) {
                continue;
            }
            ...
            if let Some(similar) = similarity_graph.get(&fragment.phi_signature) {
                ...
                for &similar_phi in similar {
                    if processed.contains(&similar_phi) {
                        if let Some(base_fragment) =
                            fragments.iter().find(|f| f.phi_signature == similar_phi)
                        {
                            let additions = extract_minimal_additions(
                                &base_fragment.functions,
                                &fragment.functions,
                            );
...
fn generate_lattice_traits(lattice: &LatticeStructure) -> String {
    ...
    output.push_str(
        "    fn extract_minimal_addition(&self, base: &dyn LatticeNavigator) -> Vec<String>;\n",
    );
...
fn consume_and_refactor(fragments: &[CodeFragment]) -> LatticeStructure {
    println!(
        "🔍 Analyzing {} code fragments for consumption",
        fragments.len()
    );
...
fn main() {
    ...
    for (phi, similar) in lattice.similarity_graph.iter().take(5) {
        println!(
            "🔗 φ {} connects to {} similar fragments",
            phi,
            similar.len()
        );
    }
    ...
    println!(
        "📊 Consumed {} fragments into {} layered traits",
        lattice.consumed_code.len(),
        lattice.layers.len()
    );
}
