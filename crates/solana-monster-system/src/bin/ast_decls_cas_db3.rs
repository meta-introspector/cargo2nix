...
impl ASTDeclsDB3 {
    ...
    fn extract_struct_decl(&mut self, line: &str, file: &str, line_num: usize, content: &str) {
        let struct_start = if line.starts_with("pub struct ") { 11 } else { 7 };
    ...
    fn extract_trait_decl(&mut self, line: &str, file: &str, line_num: usize, content: &str) {
        let trait_start = if line.starts_with("pub trait ") { 10 } else { 6 };
    ...
    fn calculate_decl_cas(&self, name: &str, decl_type: &str, deps: &[String], content: &str) -> u64 {
    ...
    fn add_declaration(&mut self, decl: ASTDeclaration) {
        ...
        println!("  {} {} → CAS: {} ({} deps)", decl.decl_type, name, cas, decl.dependencies.len());
        
        self.declarations.insert(cas, decl);
        self.name_to_cas.entry(name).or_insert_with(Vec::new).push(cas);
    }
    ...
    fn show_database(&self) {
        ...
        for (cas, decl) in self.declarations.iter().take(10) {
            println!("🔧 {} {} (CAS: {}) at {}:{}", 
                     decl.decl_type, decl.name, cas, 
                     decl.source_file.split('/').last().unwrap_or(""), decl.line_number);
        }
    }
}
