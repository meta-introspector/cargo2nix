...
use solana_monster_system::{MiniZincSolver, ModularFormEncoding, ModularFormZKP};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        phi_encoding.topological_invariant
    );
    println!(
        "Arithmetic constraint: {} (mod 24)",
        phi_encoding.arithmetic_constraint
    );
    println!(
        "Symmetry group sum: {}",
        phi_encoding.symmetry_group.iter().sum::<i64>() % 24
    );
    ...
    for (i, invariant) in zkp_system.invariants.iter().enumerate() {
        println!(
            "Invariant {}: {:?} = {}",
            i, invariant.invariant_type, invariant.value
        );
