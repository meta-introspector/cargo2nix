...
impl CASStorage {
    ...
    fn add_node(&mut self, name: &str, content: &str, dependencies: Vec<&str>) {
        ...
        println!("NODE: {} → CAS address: {} (from {} deps)", name, cas_address, dependencies.len());
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Intermediate nodes
    cas.add_node("serde::Serialize", "pub trait Serialize { ... }", vec!["std::HashMap"]);
    cas.add_node("tokio::Runtime", "pub struct Runtime { ... }", vec!["std::Vec"]);
    
    // Complex nodes with multiple dependencies
    cas.add_node("our::meme_pda", 
                 "pub fn create_meme_pda() { ... }", 
                 vec!["std::HashMap", "serde::Serialize"]);
    
    cas.add_node("our::monster_solver", 
    ...
    if let Some(node) = cas.get_by_name("our::monster_solver") {
        println!("Retrieved by name: {} → CAS: {}", node.name, node.cas_address);
