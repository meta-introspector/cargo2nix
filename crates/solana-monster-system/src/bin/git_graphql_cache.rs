...
impl GitGraphQLCache {
    ...
    fn query_git_status(&mut self) -> String {
            ...
            .output()
        {
            ...
            self.cache
                .git_status
                .insert("current".to_string(), status.to_string());
            ...
            .output()
        {
            ...
            for line in log.lines() {
                ...
                if !parts.is_empty() {
                    ...
                    if let Ok(obj_output) =
                        Command::new("git").args(&["cat-file", "-p", hash]).output()
                    {
                        let content = String::from_utf8_lossy(&obj_output.stdout);

                        self.git_objects.insert(
                            hash.to_string(),
                            GitObject {
                                hash: hash.to_string(),
    ...
    fn query_ingest_metadata(&mut self) -> String {
        ...
        if let Ok(content) = std::fs::read_to_string(".gitmodules") {
            self.cache
                .gitmodules
                .insert("root".to_string(), content.clone());
            result.push_str(&format!(
                "    gitmodules = \"{}...\" (cached)\n",
                &content[..50.min(content.len())]
            ));
