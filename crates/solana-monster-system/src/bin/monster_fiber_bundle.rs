...
impl MonsterFiberBundle {
    ...
    fn count_binary_structures(&self, content: &str) -> u64 {
        content.matches("bool").count() as u64 + content.matches("Option").count() as u64
    }

    fn count_ternary_structures(&self, content: &str) -> u64 {
        content.matches("enum").count() as u64 + content.matches("match").count() as u64
    }
    ...
    fn classify_bundle_section(&self, key: &str) -> String {
        if key.contains("cargo:") {
            "cargo_section".to_string()
        } else if key.contains("git_repo:") {
            "repo_section".to_string()
        } else if key.contains("ast:") {
            "ast_section".to_string()
        } else {
