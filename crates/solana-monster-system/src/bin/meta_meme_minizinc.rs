...
fn main() {
    ...
    println!("Population: {}, Generations: {}", population_size, generations);
...
fn print_population_stats(system: &MetaMemeSporeSystem) {
    ...
    let total_ram = system.spores.iter()
        .map(|s| s.resource_allocation.ram_bytes as u32)
        .sum::<u32>();
    let total_tokens = system.spores.iter()
...
fn print_top_spores(system: &MetaMemeSporeSystem, count: usize) {
    ...
    for (i, spore) in sorted_spores.iter().take(count).enumerate() {
        println!("  #{}: Fitness: {:.4}, Monster Element: {}, Gödel: {}, Tokens: {}", 
                 i + 1, 
                 spore.fitness, 
...
fn generate_optimization_report(system: &MetaMemeSporeSystem) {
    ...
    let avg_fitness = system.spores.iter().map(|s| s.fitness).sum::<f64>() / system.spores.len() as f64;
    ...
    let report = format!(
        ...
        best_spore.meme_tokens.iter()
            .take(3)
            .enumerate()
            .map(|(i, token)| format!("{}. {} (Value: {:.3})", i + 1, token.lisp_expression, token.value))
            .collect::<Vec<_>>()
            .join("\n")
    );
    ...
    println!("  Estimated Money Generation: ${:.2}", best_spore.fitness * 100.0);
