...
use std::collections::{HashMap, HashSet, VecDeque}; // NEW: For graph and topological sort

use crate::analysis_types::{IngestionChunk, IngestionFileDescriptor, ProjectFileAnalysis};
use crate::hasher::calculate_content_id;
use crate::file_ingestion::{self, ingest_single_file}; // NEW: To ingest files as needed
...
fn get_project_file_analysis(db: &DB, file_path_str: &str, project_root: &Path) -> Result<ProjectFileAnalysis> {
    ...
    if let Some(content_hash_bytes) = db.get(format!("git_tree_entry:{}", file_path_str).as_bytes())? {
    ...
    eprintln!("File analysis not found for {} in RocksDB, ingesting single file...", file_path_str);
...
pub fn generate_ingestion_plan(db: &DB, project_root: &Path, seed_path_str: &str) -> Result<()> {
    ...
    let normalized_seed_path = seed_path_buf.canonicalize()

                                            .context(format!("Failed to canonicalize seed path: {}", seed_path_str))?
    ...
    while let Some(current_file_path_str) = q.pop_front() {
        ...
        if let Some(uses) = analysis.uses {
            ...
            for use_stmt in uses {
                ...
                if use_stmt.contains("::") { // Simple heuristic for now
                    ...
                    } else if let Some(resolved) = resolve_rust_path(&PathBuf::from(&current_file_path_str).parent().unwrap_or(Path::new("")), &potential_path) {
        ...
        if let Some(mods) = analysis.mods {
            ...
            for mod_name in mods {
                ...
                if mod_path.exists() {
                     ...
                     direct_dependencies.insert(mod_path.canonicalize()?.to_string_lossy().to_string());
        ...
        for dep_path_str in direct_dependencies {
            ...
            let normalized_dep_path = PathBuf::from(&dep_path_str).canonicalize()

                                            .context(format!("Failed to canonicalize dependency path: {}", dep_path_str))?
            ...
            graph.entry(current_file_path_str.clone()).or_default().insert(normalized_dep_path.clone());
    ...
    let mut current_chunk_size_bytes: usize = 0;
        ...
        let descriptor_serialized_len = serde_json::to_string(&descriptor)
                                            ...
                                            .unwrap_or_else(|_| "".to_string()).len();

        

        if current_chunk_size_bytes + descriptor_serialized_len > CHUNK_TARGET_SIZE_BYTES && !current_chunk_files.is_empty() {

            let chunk_id = calculate_content_id(serde_json::to_string(&current_chunk_files)?.as_bytes());
            ...
            db.put(db_key.as_bytes(), serde_json::to_string(&chunks.last().unwrap())?.as_bytes())

                .context(format!("Failed to write ingestion plan chunk {} to RocksDB", chunk_id))?;
